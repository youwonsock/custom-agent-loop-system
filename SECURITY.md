...
High-severity production dependency audits, generated-file drift checks, and unpacked VSIX smoke tests are fail-closed release gates. A registry or audit-service outage may be handled only by rerunning the bounded CI job after service recovery; it must not be bypassed or reclassified as a successful scan. Lockfile changes require a fresh audit. The Linux VSIX built on Node 20 is also reverified unchanged on Node 18 so the packaged artifact, rather than only the source tree, exercises the declared runtime floor.
