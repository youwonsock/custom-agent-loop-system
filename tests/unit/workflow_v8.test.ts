...
import type { DefinitionSourceBundle } from "../../src/domain/workflow";
...
async function initTestRunStorage(root: string, runId: string): Promise<void> {
  await initRunStorage(root, runId, {
    paths: {
      sessionsRoot: "runs",
...
async function loadTestSource(): Promise<DefinitionSourceBundle> {
  return structuredClone(await loadDefinitionSource(process.cwd()));
}
...
test("v8 default definitions compile into one frozen deterministic bundle", async () => {
  ...
  assert.equal(bundle.nodes.VERIFY.kind, "verification");
  assert.equal(bundle.transitions.TEST.prepared, "VERIFY");
  assert.equal(bundle.transitions.VERIFY.pass, "QA_REVIEW");
  assert.equal(bundle.transitions.VERIFY.fail, "IMPLEMENTATION");
  assert.deepEqual(bundle.cyclePolicy.completionNodeIds, ["VERIFY", "QA_REVIEW", "MASTER_APPROVAL"]);
  assert.deepEqual(bundle.applicationPolicy, {
    interruptNodeId: "INTERRUPT",
    blockedTerminalId: "BLOCKED",
    implementationNodeId: "IMPLEMENTATION",
    testNodeId: "TEST",
    verificationNodeId: "VERIFY",
    qaNodeId: "QA_REVIEW",
    completionApprovalNodeId: "MASTER_APPROVAL",
  });
...
test("v8 compiler rejects undeclared signals, access expansion, and approval bypass", async () => {
  const source = await loadDefinitionSource(process.cwd());
  ...
  badSignal.workflow.transitions.find(
    (rule) => rule.from === "TEST" && rule.on === "prepared"
  )!.on = "maybe";
...
test("v8 compiler rejects superseded workflow contracts and verification bypasses", async () => {
  const source = await loadDefinitionSource(process.cwd());
  const registries = createDefaultDefinitionRegistries();

  const oldSchema = structuredClone(source) as any;
  oldSchema.workflow.schemaVersion = 1;
  assert.throws(() => compileWorkflow(oldSchema, registries), /workflow definitions must use schemaVersion 2/u);

  const singularCompletion = structuredClone(source) as any;
  delete singularCompletion.workflow.cyclePolicy.completionNodeIds;
  singularCompletion.workflow.cyclePolicy.completionNodeId = "VERIFY";
  assert.throws(() => compileWorkflow(singularCompletion, registries), /completionNodeIds|undefined/u);

  const noVerification = structuredClone(source) as any;
  noVerification.workflow.nodes = noVerification.workflow.nodes.filter((node: { id: string }) => node.id !== "VERIFY");
  noVerification.workflow.transitions = noVerification.workflow.transitions.filter(
    (transition: { from: string; to: string }) => transition.from !== "VERIFY" && transition.to !== "VERIFY"
  );
  noVerification.workflow.applicationPolicy.verificationNodeId = "QA_REVIEW";
  noVerification.workflow.cyclePolicy.completionNodeIds = ["QA_REVIEW", "MASTER_APPROVAL"];
  assert.throws(() => compileWorkflow(noVerification, registries), /verification node|Verification success policy|no transition/u);
});
...
test("one TaskDefinition can be reused by multiple agents and nodes", async () => {
  ...
  const bundle = compileWorkflow(source, createDefaultDefinitionRegistries());
  assert.equal(bundle.nodes.PLANNING.kind, "task");
  assert.equal(bundle.nodes.PLAN_REFINEMENT.kind, "task");
...
test("JSON-only task runner rejects legacy completion text after one tool-free recovery", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v8-json-"));
  try {
    const source = await loadTestSource();
    const registries = createDefaultDefinitionRegistries();
...
test("workflow runner resumes PLAN_APPROVAL without replaying planning and preserves activations", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v8-run-"));
  try {
    const source = await loadTestSource();
    const registries = createDefaultDefinitionRegistries();
    const bundle = compileWorkflow(source, registries);
    ...
    const runtime = new ScriptedRuntime((request) => {
      ...
      switch (request.taskId) {
          ...
          assistantText = envelope(
            "success",
            {
              choices: [1, 2, 3].map((id) => ({
                ...
                verification: {
                  commands: [{ id: "tests", label: "Tests", executable: "node", args: ["--version"], cwd: ".", timeoutMs: 10_000, requirementIds: ["REQ-001"] }],
        ...
        case "implement_changes":
          ...
          break;
        case "run_tests":
          assistantText = envelope("prepared", {
            verdict: "pass",
    ...
    await initTestRunStorage(temporaryRoot, initial.runId);
    ...
    const verificationContracts = new VerificationContractService(new FileWorkspaceIntegrity(), artifacts);
    const commands = new CommandService(repository, reducer, projection, undefined, verificationContracts);
    ...
    assert.equal(Object.keys(completed.nodeExecutions).length, 3);
    assert.equal(new Set(Object.keys(completed.nodeExecutions)).size, 3);
    ...
    assert.equal(implementationActivations.length, 1);
    assert.equal(implementationActivations[0].status, "unknown_mutation");
...
test("provider access approval retries the same activation with a new attempt", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v8-access-"));
  try {
    const source = await loadTestSource();
    const registries = createDefaultDefinitionRegistries();
    const bundle = compileWorkflow(source, registries);
...
test("queued STOP is reduced at a checkpoint before another provider starts", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v8-control-"));
  try {
    const source = await loadDefinitionSource(process.cwd());
    const registries = createDefaultDefinitionRegistries();
    const bundle = compileWorkflow(source, registries);
