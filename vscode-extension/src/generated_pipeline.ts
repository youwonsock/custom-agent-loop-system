// Generated by scripts/sync-runtime-defaults.js. Edit pipeline.ts.
...
export function validatePipeline(input: PipelineDefinition): PipelineDefinition {
  ...
  for (const stage of input.stages) {
    ...
    if (TERMINAL_TARGETS.has(stage.id)) throw new Error(`Reserved pipeline stage id: ${stage.id}`);
  ...
  // Cycles are valid, but every stage must have a possible exit to a terminal.
  const canExit = new Set(TERMINAL_TARGETS);
  let changed = true;
  while (changed) {
    changed = false;
    for (const stage of input.stages) {
      if (!canExit.has(stage.id) && (canExit.has(stage.onSuccess) || canExit.has(stage.onFailure))) {
        canExit.add(stage.id);
        changed = true;
      }
    }
  }
  const trapped = input.stages.filter((stage) => !canExit.has(stage.id));
  if (trapped.length) throw new Error(`Stages have no path to SUCCESS or PAUSED: ${trapped.map((stage) => stage.id).join(", ")}`);
  return input;
}

/** Session mappings are the resolved snapshot; file defaults are only a fallback. */
export function resolveRoleExecutionSettings(
  role: PipelineRole,
  settings: {
...
): { providerId: string; model: string; variant: string | undefined } {
  const model = settings.modelMapping[role.id] ?? role.model ?? settings.modelMapping[role.modelRole];
  if (!model) throw new Error(`No model configured for role ${role.id} (modelRole=${role.modelRole}).`);
  return {
    providerId: settings.providerMapping[role.id] ?? role.provider ?? settings.providerMapping[role.modelRole] ?? settings.cliProfile,
    model,
    variant: settings.variantMapping[role.id] ?? role.variant ?? settings.variantMapping[role.modelRole],
  };
}

/** Independent of countsIteration and chosen transition outcomes. */
export function stageExecutionBudget(pipeline: PipelineDefinition, maxIterations: number): number {
  const iterations = Number.isSafeInteger(maxIterations) && maxIterations > 0 ? maxIterations : 1;
  return iterations * Math.max(1, pipeline.stages.length) * 2;
}

export function consumeStageExecutionBudget(state: {
  ...
  if (state.stageExecutions >= state.stageExecutionLimit) return false;
