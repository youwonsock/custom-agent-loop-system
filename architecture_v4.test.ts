...
test("v4 production architecture has one model runner and no legacy workflow implementation", () => {
...
test("VS Code consumes a projection and cannot author the aggregate", () => {
  const extensionRoot = path.join(root, "vscode-extension", "src");
  assert.equal(fs.existsSync(path.join(extensionRoot, "aggregateStore.ts")), false);
  const store = fs.readFileSync(path.join(extensionRoot, "stateStore.ts"), "utf8");
  const client = fs.readFileSync(path.join(extensionRoot, "loopClient.ts"), "utf8");
  assert.match(store, /projectionSchemaVersion === 1/u);
  assert.doesNotMatch(store, /async updateState|commitOffline|RunAggregate/u);
  assert.doesNotMatch(store, /loop_state\.json|enqueueControlRequest|legacy_text/u);
  assert.match(client, /invokeCoreCommand/u);
  assert.doesNotMatch(client, /enqueueControlRequest|markSessionStopped/u);
});
