...
test("development desktop renders the operator console", async () => {
  ...
  const electronApp = await electron.launch({
    executablePath: path.resolve(__dirname, "..", "..", "desktop-app", "node_modules", "electron", "dist", process.platform === "win32" ? "electron.exe" : "electron"),
    args: [path.resolve(__dirname, "..", "..", "desktop-app", ".e2e", "main", "main.js")],
    env: {
      ...process.env,
      APPDATA: appData,
      ...
      ELECTRON_IS_DEV: "1",
      AGENT_LOOP_E2E: "1",
      AGENT_LOOP_APP_ROOT: path.resolve(__dirname, "..", "..", "desktop-app"),
    },
  });
  try {
