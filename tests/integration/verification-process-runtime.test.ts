...
import { VerificationProcessRuntime } from "../../src/runtime/verification-process-runtime";

const repositoryHelper = path.resolve(process.cwd(), "native/bin/win32-x64/verification-host.exe");
...
// The test runtime is compiled under .build/test-dist, while the production
// runtime intentionally only searches its npm and desktop package layouts.
// Point the test at the repository helper explicitly so the real Windows Job
// Object path is exercised instead of silently skipping all process tests.
if (process.platform === "win32" && !process.env.AGENT_LOOP_VERIFICATION_HELPER && existsSync(repositoryHelper)) {
  process.env.AGENT_LOOP_VERIFICATION_HELPER = repositoryHelper;
}
