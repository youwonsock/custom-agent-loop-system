...
const distSource = path.join(root, "dist", "core");
if (!fs.existsSync(distSource)) throw new Error("dist/core does not exist; run npm run build first.");
...
const stagedConfig = path.join(destination, "config");
fs.rmSync(stagedConfig, { recursive: true, force: true });
fs.mkdirSync(stagedConfig, { recursive: true });
for (const file of [
  "agents.json", "agents.schema.json", "tasks.json", "tasks.schema.json",
  "workflow.json", "workflow.schema.json", "loop_config.json", "loop_config.schema.json",
  "protocol_contract.json", "runtime_defaults.json",
]) {
  fs.copyFileSync(path.join(configSource, file), path.join(stagedConfig, file));
}
