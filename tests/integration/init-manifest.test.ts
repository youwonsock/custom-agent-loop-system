...
} from "../../src/application/init-manifest";
...
test("validateInitializedRoots binds the packaged and copied definitions to the manifest", async () => {
  ...
  try {
    ...
    const packagedRoot = path.join(process.cwd(), "config");
    for (const fileName of INIT_DEFINITION_FILES) {
      await cp(path.join(packagedRoot, fileName), path.join(configRoot, fileName));
    }
    const digest = await hashDefinitionFiles(packagedRoot);
