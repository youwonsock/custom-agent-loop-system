...
test("v7 default definitions compile into one frozen deterministic bundle", async () => {
...
test("v7 compiler rejects undeclared signals, access expansion, and approval bypass", async () => {
  const source = await loadDefinitionSource(process.cwd());
...
test("v7 compiler rejects superseded workflow contracts and verification bypasses", async () => {
  const source = await loadDefinitionSource(process.cwd());
  const registries = createDefaultDefinitionRegistries();
...
test("JSON-only task runner rejects legacy completion text after one tool-free recovery", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v7-json-"));
  try {
...
test("workflow runner resumes PLAN_APPROVAL without replaying planning and preserves activations", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v7-run-"));
  try {
    const source = await loadTestSource();
    const registries = createDefaultDefinitionRegistries();
    const bundle = compileWorkflow(source, registries);
...
test("provider access approval retries the same activation with a new attempt", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v7-access-"));
  try {
    const source = await loadTestSource();
    const registries = createDefaultDefinitionRegistries();
    const bundle = compileWorkflow(source, registries);
...
test("queued STOP is reduced at a checkpoint before another provider starts", async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "agent-loop-v7-control-"));
  try {
    const source = await loadDefinitionSource(process.cwd());
    const registries = createDefaultDefinitionRegistries();
    const bundle = compileWorkflow(source, registries);
